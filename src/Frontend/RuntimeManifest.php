...
/**
 * Builds the demo runtime manifest used by [storyboard_live_demo].
 *
 * Frame sets are constructed once per request and reused across all variant
 * builds to avoid redundant URL-array construction.
 */
final class RuntimeManifest {
	...
	/**
	 * Build a single responsive variant definition.
	 *
	 * @param string                         $id           Variant id.
	 * @param array<string,array<string,mixed>> $frame_sets Full frame-set map (passed in to avoid repeated construction).
	 * @param string                         $frame_set_id Key into $frame_sets.
	 * @param int                            $width        Display width.
	 * @param int                            $height       Display height.
	 * @param int                            $min_width    Min-width media condition.
	 * @param int                            $max_width    Max-width media condition.
	 * @param string                         $orientation  Orientation condition.
	 * @param int                            $priority     Variant priority.
	 * @param int                            $scroll_length Scroll-space length in vh.
	 * @param float                          $dpr_cap      Device-pixel-ratio cap.
	 * @param string                         $layout       Live-layout key.
	 * @return array<string,mixed>
	 */
	private function build_variant( $id, array $frame_sets, $frame_set_id, $width, $height, $min_width, $max_width, $orientation, $priority, $scroll_length, $dpr_cap, $layout ) {
		$urls = $frame_sets[ $frame_set_id ]['urls'];
		return array(
			'id'               => sanitize_key( $id ),
	...
	/**
	 * Build the full demo manifest for a runtime instance.
	 *
	 * @param string $instance_id Unique DOM instance id.
	 * @return array<string,mixed>
	 */
	public function build_demo_manifest( $instance_id ) {
		// Build frame sets once and reuse across all variants.
		$frame_sets = $this->build_frame_sets();

		$variants   = array(
			$this->build_variant( 'desktop', $frame_sets, self::DESKTOP_FRAME_SET, 1280, 720, 1180, 10000, 'any', 10, 500, 1.5, 'desktop' ),
			$this->build_variant( 'tablet', $frame_sets, self::DESKTOP_FRAME_SET, 1280, 720, 768, 1179, 'any', 20, 240, 1.3, 'tablet' ),
			$this->build_variant( 'mobile-portrait', $frame_sets, self::MOBILE_FRAME_SET, 720, 1280, 0, 767, 'portrait', 40, 150, 1.1, 'mobile' ),
			$this->build_variant( 'mobile-landscape', $frame_sets, self::DESKTOP_FRAME_SET, 1280, 720, 0, 959, 'landscape', 50, 120, 1.1, 'mobile-landscape' ),
		);
	...
	/**
	 * Return preload candidates for the demo manifest's entry frames.
	 *
